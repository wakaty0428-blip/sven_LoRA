...
def save_model(model, path, args):
    ...
    # lora save(lora version)
        ...
        # ------------------------------------------------------
        # SEC adapter = adapter_name "default"
        # ------------------------------------------------------
        sec_state = {
            k: v.cpu()
        ...
        torch.save(sec_state, os.path.join(sec_dir, "adapter_model.bin"))
        # print(f"[SAVE] SEC adapter → {sec_dir} ({len(sec_state)} params)")


        # --- Save SEC adapter_config.json ---
        if "default" in model.peft_config:
            sec_cfg = model.peft_config["default"]
            with open(os.path.join(sec_dir, "adapter_config.json"), "w") as f:
                json.dump(sec_cfg.to_dict(), f, indent=2)
            # print(f"[SAVE] SEC adapter_config.json → {sec_dir}")

        # ------------------------------------------------------
        # VUL adapter = adapter_name "vul"
        # ------------------------------------------------------
        vul_state = {
            k: v.cpu()
            for k, v in model.named_parameters()
            if ("lora_A.vul" in k or "lora_B.vul" in k)
        }
        vul_dir = os.path.join(path, "vul")
        os.makedirs(vul_dir, exist_ok=True)
        torch.save(vul_state, os.path.join(vul_dir, "adapter_model.bin"))
        # print(f"[SAVE] VUL adapter → {vul_dir} ({len(vul_state)} params)")

        # --- Save VUL adapter_config.json ---
        if "vul" in model.peft_config:
            vul_cfg = model.peft_config["vul"]
            with open(os.path.join(vul_dir, "adapter_config.json"), "w") as f:
                json.dump(vul_cfg.to_dict(), f, indent=2)
            # print(f"[SAVE] VUL adapter_config.json → {vul_dir}")
...
def load_model(model_type, path, is_training, args):
    ...
    # lora version
        ...
        # ------------------------
        # Base model: pretrain_dir
        # ------------------------
        lm_path = args.pretrain_dir
        config = config_from_pretrained(lm_path, lm_path)
        
        # load base model as normal LM
        base_model = model_from_pretrained(lm_path, 'lm', config)
